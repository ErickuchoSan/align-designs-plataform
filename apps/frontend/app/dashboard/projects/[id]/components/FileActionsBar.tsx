...
export default function FileActionsBar({
  ...
  return (
    <div className="mb-6">
      ...
      {isWaitingPayment && (
        <div className={`mb-4 rounded-lg border-l-4 p-4 shadow-sm ${project.initialAmountRequired === null
            ? 'bg-amber-50 border-amber-500'
            : 'bg-yellow-50 border-yellow-500'
          }`}>
          <div className="flex items-start">
            <div className="flex-shrink-0">
              <svg className={`h-5 w-5 ${project.initialAmountRequired === null ? 'text-amber-400' : 'text-yellow-400'}`} fill="currentColor" viewBox="0 0 20 20">
              ...
              {project.initialAmountRequired === null ? (
                <>
                  ...
                  <p className="mt-1 text-sm text-amber-700">
                    This project cannot be activated because the <strong>Initial Payment Amount</strong> is not set.
                    Please edit the project details to configure payment terms.
                  </p>
                </>
              ) : (
                <>
