...
function MobileFileCard({
  ...
  return (
    <div className="bg-white rounded-2xl p-4">
      {/* File Header */}
      <div className="flex items-start gap-3 mb-3 pb-3 border-b border-[#D0C5B2]/20">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-[#1B1C1A] break-words">
            {file.filename ? file.originalName : 'Comment Only'}
          </div>
          <div className="flex items-center gap-2 mt-1">
            {file.filename ? (
              <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-[#F5F4F0] text-[#1B1C1A]">
                {fileExtension}
              </span>
            ) : (
      ...
      {/* File Details */}
      {file.comment && (
        <div className="mb-3 pb-3 border-b border-[#D0C5B2]/20">
          <div className="text-xs text-gray-500 mb-1">Comment</div>
          <div className="text-sm text-[#6B6A65] line-clamp-2">{sanitizeText(file.comment)}</div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 mb-3 text-xs">
        <div>
          ...
          <div className="text-sm text-[#1B1C1A] font-medium">{uploaderName}</div>
        </div>
        <div>
          <div className="text-gray-500">Date</div>
          <div className="text-sm text-[#1B1C1A]">{formattedDate}</div>
        </div>
      </div>

      {/* Actions */}
      <div className="flex flex-wrap gap-2 pt-3 border-t border-[#D0C5B2]/20">
        {file.filename && (
          <>
            <ActionButton
...
const ActionButton = memo(function ActionButton({
  ...
  const hoverColor = variant === 'danger' ? 'hover:text-red-600' : 'hover:text-[#1B1C1A]';

  return (
    <button
      onClick={onClick}
      className={`flex-1 min-w-[calc(50%-0.25rem)] flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium text-[#6B6A65] ${hoverColor} hover:bg-[#F5F4F0] rounded-lg transition-colors border border-[#D0C5B2]/20`}
