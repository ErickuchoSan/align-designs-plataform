...
import { UsersTable, UsersCards, UserModals } from '.';
...
export default function UsersPageLayout({
  ...
  const filteredUsers = useMemo(
    () => usersHook.users.filter((usr) => usr.role === role),
    [usersHook.users, role]
  );
  ...
  const handleCloseCreateModal = () => {
    ...
    usersHook.setFormData({ email: '', firstName: '', lastName: '', phone: '', role });
  };

  return (
    <>
      <div className="flex flex-col h-full">
        <DashboardHeader title={title} showBackButton backUrl="/dashboard" />
