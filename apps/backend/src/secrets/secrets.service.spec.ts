...
describe('SecretsService', () => {
    ...
    describe('getSecretOrThrow', () => {
        ...
        it('should throw if secret not found (delegated)', async () => {
            mockConfigService.get.mockReturnValue('false');
            service.onModuleInit();
            mockEnvSecretsManager.getSecretOrThrow.mockRejectedValue(new Error('Secret key not found'));

            await expect(service.getSecretOrThrow('MISSING_KEY')).rejects.toThrow('Secret key not found');
        });
    });
});
