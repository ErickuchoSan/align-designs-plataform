...
export class AnalyticsService {
    ...
    async getClientFinancials(clientId: string) {
        ...
        const totalBilled = invoices.reduce((sum: number, inv) => sum + Number(inv.totalAmount), 0);
        const totalPaid = invoices.reduce((sum: number, inv) => sum + Number(inv.amountPaid), 0);
        const outstanding = totalBilled - totalPaid;
        const projectCount = await this.prisma.project.count({ where: { clientId } });
    ...
    async getEmployeePerformance() {
        ...
        for (const emp of employees) {
            ...
            const firstTryRate = totalDeliveries > 0 ? (firstTryCount / totalDeliveries) * 100 : 0;
            const avgRejections = totalDeliveries > 0 ? (totalRejections / totalDeliveries) : 0;
            ...
            performanceData.push({
                ...
                metrics: {
                    ...
                    efficiencyPerDay: parseFloat(efficiencyPerDay.toFixed(2)),
                    firstTryRate: parseFloat(firstTryRate.toFixed(1)),
                    avgRejections: parseFloat(avgRejections.toFixed(1))
                }
            });
        }
