...
export class TimeTrackingService {
  ...
  async startTracking(
    ...
    if (existing) {
      this.logger.warn(
        `Time tracking already active for cycle ${feedbackCycleId}`,
      );
      return existing;
    }
  ...
  private calculateOfficialStartTime(inputDate: Date): Date {
    ...
    if (hour < 12) {
      ...
      this.logger.log(
        `Timer Rule Applied (<12PM): Input ${inputDate.toISOString()} -> Start ${nextDay.toISOString()}`,
      );
      return nextDay;
    }
    ...
    this.logger.log(
      `Timer Rule Applied (>=12PM): Input ${inputDate.toISOString()} -> Start ${inputDate.toISOString()}`,
    );
    return inputDate;
  }
  ...
  private calculateDurationDays(start: Date, end: Date): number {
    ...
    return Number.parseFloat(diffDays.toFixed(2));
  }

  /**
   * Calculate duration breakdown (days, hours, minutes)
   */
  private calculateDurationBreakdown(
    start: Date,
    end: Date,
  ): { days: number; hours: number; minutes: number; totalDays: number } {
    const diffTime = Math.abs(end.getTime() - start.getTime());
  ...
  async getProjectStats(projectId: string, userId?: string, userRole?: string) {
    ...
    const endDate =
      project.status === 'COMPLETED' ? project.updatedAt : new Date();
    const durationBreakdown = this.calculateDurationBreakdown(
      startDate,
      endDate,
    );
    ...
    const totalRejections = trackings.filter(
      (t) => t.cycle?.status === 'rejected',
    ).length;
    ...
    const averageCycleDuration =
      totalCycles > 0
        ? trackings.reduce((sum, t) => sum + (t.durationDays || 0), 0) /
          totalCycles
        : 0;
    ...
    return {
      ...
      totalCycles,
      averageCycleDuration: Number.parseFloat(averageCycleDuration.toFixed(1)),
      rejectionRate: Number.parseFloat(rejectionRate.toFixed(2)),
      totalRejections,
    };
  }
}
