...
export class PaymentApprovalService {
  ...
  async approvePayment(
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (payment.invoiceId && payment.invoice) {
        const newInvoiceAmountPaid =
          Number(payment.invoice.amountPaid) + finalAmount;
        const invoiceTotal = Number(payment.invoice.totalAmount);
    ...
    try {
      const validation = await this.projectStatusService.canActivateProject(
        payment.project.id,
      );
      if (validation.canActivate) {
        await this.projectStatusService.activateProject(payment.project.id);
        this.logger.log(
          `Project ${payment.project.id} auto-activated after payment approval`,
        );
      }
    } catch (error) {
      // Log but don't fail - the payment is already approved
      this.logger.warn(
        `Auto-activation check failed for project ${payment.project.id}: ${error}`,
      );
    }
  ...
  private async notifyPaymentApproved(
    ...
    if (overpaymentWarning) {
      notificationMessage +=
        ' Nota: El pago excede el monto de la factura. El saldo a favor será aplicado a futuros servicios.';
    }
