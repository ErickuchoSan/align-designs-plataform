...
export class EmployeePaymentsController {
  ...
  @ApiOperation({
    summary: 'Get all employee payments (Admin sees all, Employee sees own)',
  })
  findAll(
    @CurrentUser() user: UserPayload,
  ...
  getPendingItems(
    ...
    return this.employeePaymentsService.getPendingPaymentItems(
      projectId,
      employeeId,
    );
  }
  ...
  @Roles(Role.ADMIN)
  @ApiOperation({
    summary: 'Update employee payment (Admin only, only PENDING)',
  })
  update(
    @Param('id', ParseUUIDPipe) id: string,
  ...
  reject(
    ...
    return this.employeePaymentsService.reject(
      id,
      rejectionReason,
      user.userId,
    );
  ...
  @Roles(Role.ADMIN)
  @ApiOperation({
    summary: 'Delete employee payment (Admin only, only PENDING/REJECTED)',
  })
  ...
  async getReceiptUrl(
    ...
    const url = await this.employeePaymentsService.getReceiptDownloadUrl(
      id,
      user.userId,
      user.role,
    );
