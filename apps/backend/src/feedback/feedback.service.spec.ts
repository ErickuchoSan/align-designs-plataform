...
describe('FeedbackService', () => {
    ...
    describe('createFeedbackCycle', () => {
        it('should return existing open cycle if one exists', async () => {
            ...
            const result = await service.createFeedbackCycle(mockProjectId, mockEmployeeId);
    ...
    describe('submitFeedbackCycle', () => {
        ...
        it('should throw BadRequest if cycle is not open', async () => {
            prismaService.feedbackCycle.findUnique.mockResolvedValue({ ...mockCycle, status: 'submitted' });

            await expect(service.submitFeedbackCycle(mockCycleId, 'file-123'))
    ...
    describe('approveFeedbackCycle', () => {
        ...
        it('should throw BadRequest if cycle is not submitted', async () => {
            prismaService.feedbackCycle.findUnique.mockResolvedValue({ ...mockCycle, status: 'open' });

            await expect(service.approveFeedbackCycle(mockCycleId))
                .rejects.toThrow(BadRequestException);
        });
    });
