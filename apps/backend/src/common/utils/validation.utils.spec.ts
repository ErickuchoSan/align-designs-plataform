...
describe('PhoneValidationUtils', () => {
  ...
  describe('formatToE164', () => {
    ...
    it('should return null for invalid phone numbers', () => {
      expect(PhoneValidationUtils.formatToE164('123')).toBeNull();
      expect(PhoneValidationUtils.formatToE164('abcdef')).toBeNull();
    });
  });
...
describe('PasswordValidationUtils', () => {
  describe('validatePassword', () => {
    ...
    it('should reject passwords without numbers', () => {
      const result = PasswordValidationUtils.validatePassword('MyPassword!@#x');
      expect(result.isValid).toBe(false);
      expect(result.error).toContain('number');
    });

    it('should reject passwords without special characters', () => {
      const result = PasswordValidationUtils.validatePassword('MyPassword1234');
      expect(result.isValid).toBe(false);
    ...
    it('should reject passwords with sequential characters', () => {
      // Use a password without common patterns but with sequential chars
      const result = PasswordValidationUtils.validatePassword('MyP@ssw0rdxyz!');
      expect(result.isValid).toBe(false);
  ...
  describe('calculatePasswordStrength', () => {
    it('should calculate strength correctly based on requirements', () => {
      // 'abc' = lowercase only = score 1
      const veryWeak = PasswordValidationUtils.calculatePasswordStrength('abc');
      expect(veryWeak.score).toBe(1);
      expect(veryWeak.label).toBe('Very Weak');

      // 'Abc' = uppercase + lowercase = score 2
      const weak = PasswordValidationUtils.calculatePasswordStrength('Abc');
      expect(weak.score).toBe(2);
      expect(weak.label).toBe('Weak');

      // 'Abc123' = uppercase + lowercase + number = score 3
      const fair = PasswordValidationUtils.calculatePasswordStrength('Abc123');
      expect(fair.score).toBe(3);
      expect(fair.label).toBe('Fair');

      // 'Abc123!' = uppercase + lowercase + number + symbol = score 4
      const good = PasswordValidationUtils.calculatePasswordStrength('Abc123!');
  ...
  describe('checkPasswordContainsUserInfo', () => {
    ...
    it('should detect when password contains email local part', () => {
      // Email local part is "john.doe", password contains it
      expect(
