    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        if (cached) {
            ...
            if (cached.status === 'processing') {
                this.logger.warn(`Duplicate request detected while processing: ${cacheKey}`);
        ...
        const options = this.reflector?.get(IDEMPOTENCY_KEY, context.getHandler()) ?? {};
        ...
        return next.handle().pipe(
            tap({
                next: (response) => {
                    ...
                    this.logger.debug(`Cached response for: ${cacheKey} (TTL: ${ttlSeconds}s)`);
